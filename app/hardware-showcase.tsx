...
const parts = [
  ...
  { name: "Plate stage & Inspection chamber", short: "Inspection chamber", icon: Move, purpose: "Give each plate a repeatable position.", description: "A proposed plate carrier sits within the Inspection chamber. The chassis separates biological, logic and power areas; this illustration shows a simplified imaging assembly.", topic: "topic-16", category: "SAMPLE & ENCLOSURE", exploded: [49, 46], assembled: [51, 55] },
...
export default function HardwareShowcase() {
  ...
  return <section className="section hardware-section" id="hardware">
    <div className="hardware-intro"><div><div className="section-kicker"><span>APEX XR · INSIDE THE INSTRUMENT</span><span className="scope-tag scope-proposed">Proposed product</span></div><h2>Benchtop architecture.<br /><em>Component by component.</em></h2></div><p>Explore the hardware layers behind the proposed platform. Select a component to see its role in the system.</p></div>
    <div className="hardware-tier-note"><article><small>NEAR-TERM PROTOTYPE DIRECTION</small><h3>Lite / Core</h3><p>Begin with calibrated plate imaging, controlled lighting, human review and a traceable record. Evaluate affordable single-board or compact edge computing against measured requirements before selecting components.</p><a className="text-link" href="#products">Explore entry pathways ↗</a></article><article><small>ADVANCED RESEARCH HORIZON</small><h3>APEX XR</h3><p>The flagship dossier explores higher-resolution optics, additional sensing and automation. These are research options with separate cost and validation gates, not requirements for the first prototype.</p><a className="text-link" href="#apex">Read the proposed architecture ↗</a></article></div><div className="hardware-layout">
    ...
    <details className="reference-drawings"><summary>Original enclosure studies · 3 drawings</summary><p>User-supplied concept references. Dimensions, components and title-block metadata are not verified fabrication specifications. These represent alternative layouts; the later gantry proposal places electronics below the chamber.</p><div>{["wireframe","isometric","orthographic"].map((name,i)=><figure key={name}><a href={`/reference/${name}.jpeg`} target="_blank" rel="noreferrer"><img src={`/reference/${name}.jpeg`} loading="lazy" alt={["Wireframe enclosure study with sloped display","Three-zone isometric component study","Top, front and side enclosure views"][i]}/></a><figcaption>{["Enclosure wireframe","Three-zone architecture","Orthographic views"][i]} · supplied concept</figcaption></figure>)}</div></details><p className="hardware-boundary">Concept renders show a simplified assembly, not a manufactured device or final engineering layout. Component selection, integration and performance remain subject to validation.</p>
  </section>;
}
