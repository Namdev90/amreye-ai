...
test('raw chapters and hosted documents are retired',async()=>{for(const path of ['/library/compendium-011-015.json','/library/topic-45.json','/documents/amreye-compendium.pdf','/repository.json','/api/private/library']){const r=await fetch(base+path);assert.ok([404,410].includes(r.status),path+': '+r.status);await r.body.cancel()}});
test('public visual assets remain available',async()=>{for(const path of ['/reference/wireframe.jpeg','/hardware/apex-exploded.webp','/visuals/amreye-hero.webp']){const r=await fetch(base+path);assert.equal(r.status,200,path);await r.body.cancel()}});
