...
self.addEventListener('fetch', event => {
 ...
 if(event.request.mode === 'navigate') { event.respondWith(fetch(event.request).catch(() => new Response('<!doctype html><meta name="viewport" content="width=device-width"><title>AMR-Eye offline</title><body style="background:#031018;color:#edfffb;font:18px system-ui;padding:32px"><h1>You are offline</h1><p>Reconnect to load the latest AMR-Eye demonstration.</p><button onclick="location.reload()">Try again</button></body>', {status:503,headers:{'Content-Type':'text/html'}}))); return; }
 if(!/\.(png|svg|woff2?)$/.test(url.pathname)) return;
