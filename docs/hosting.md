...
These notes describe the retained Sites/Cloudflare integration. The public developer quick start uses `npm ci`, `npm run dev`, and `npm test`; the platform lifecycle instructions below apply only when operating that hosting environment. Optional identity and database helpers do not imply that the current public app requires sign-in or a live database.
