const CACHE='amreye-public-v9';
const OFFLINE=['/offline.html','/repository.json','/manifest.webmanifest','/amr-eye-icon.png','/brand/icon-192.png','/brand/icon-512.png'];
...
self.addEventListener('fetch',e=>{
 ...
 if(OFFLINE.includes(u.pathname))return e.respondWith(fetch(e.request).then(async r=>{if(r.ok){const c=await caches.open(CACHE);await c.put(u.pathname,r.clone())}return r}).catch(()=>caches.match(u.pathname)));
